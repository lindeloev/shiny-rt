 ...
 * TEXT:
   ...
   * The intro is loooooong. Should I just write an opinion instead?
   * More info about inverse gaussian?
   * RUN THROUGH GRAMMARLY OR SIMILAR SPELL CHECKER!
   * Add icons to shift + trunk + scale
   * Add README.md and LICENSE for GitHub
   * Color parameters red/green according to intro credo? (Probably not - too judgmental for teaching use)
...
# Types of parameters
There are three general types of parameters to describe distributions: location, scale, and shape. For reaction times, I will add one more type and call it "trunk" which is a combination of location and scale. Try dragging some sliders to see these kinds of parameters in action.

 * **Trunk:** A dominant feature of RTs is that the standard deviation increases approximately linearly with the mean [(Wagenmakers et al., 2007)](http://ejwagenmakers.com/2007/WagenmakersBrown2007.pdf). For conditions with longer reaction times (e.g., incongruent trials) you will see more varied reaction times than for conditions with shorter reaction times (e.g., congruent trials). For interpretability, it is convenient to have one parameter that changes both rather than having to regress on each separately and always interpret their interaction. Ideally, the value of the trunk parameter should be somewhere around the **center** (highest density region, e.g., the median RT or the mode RT).

 * **Location:** Moves the whole distribution towards the left (longer RTs) or right (shorter RTs) without altering its **shape** or **scale** (see below). There are two types:
   * **Location shift:** Sometimes called **non-decision time**, this denotes is the earliest possible reaction time. It's good to have a shift-parameter since no animal can percieve or respond in 0.0 ms (simple reaction time). Models without a shift parameter often end up predicting non-zero probabilities of impossibly short RTs. 
   * **Location center:** Somewhere "in the middle" of the distribution (e.g., $\mu$ in the normal distribution). I argued above that we should prefer a **trunk**-like parameter to model the center. Some distributions have a **rate** which is just the inverse of the center.

 * **Scale:** Stretches or squeezes the distribution without (severely) changing the **location** or the **trunk**. 

 * **Shape:** Changes the overall shape of the distribution without changing the **location** or **scale**. RTs have pretty similar shapes across tasks, namely a right skew. So I'd rather choose a distribution which inherently has the "right" shape than modeling this as a parameter. The **scale** parameter is often sufficient to do the fine-tuning of the skewness once a right-skewed distribution is chosen.


There are also parameters that does more than trunk, location, and scale. I will distinguish between

 * **Mechanism:** has a clear mechanistic interpretation motivated by theory.
 * **Messy:** not motivated by theory. Controversially, I will include **shape** as a messy parameter because it complicates things.

And here are three commandments:

<!-- Make this a proper quote -->

> 1. Distributions with messy parameters are bad.
> 2. Distributions with mechanism parameters are good.
> 3. Descriptive distributions with a trunk, a location shift, AND a scale parameter are good.
 ...
 * $\alpha$ (**trunk**): Boundary separation. The distance between thresholds. Longer distance --> longer and more dispersed RT before walker crosses threshold.
 * $\tau$ (**shift**): Non-decision time (ndt). The time at which the random walker starts.
 * $\beta$ (**trunk**): Bias. How close the walker starts to the upper threshold ($\beta = 0.5$ is in the middle). Higher = closer = faster and more condensed RTs.
 * $\delta$ (**trunk**): Drift Rate. The speed of the walker. Higher --> faster and more condensed RTs.
 ...
 * $\mu$ (**trunk**)
 * $shape$ (**messy**)
 * $shift$: time of earliest possible response. When $shift = 0$ it is a plain *Inverse Gaussian* or *Wald* distribution, i.e., only two parameters.
  ...
  fluidPage(
    sidebarLayout(
      ...
      sidebarPanel(
        ...
        sliderInput('sinvgauss_shape', label = NULL, pre='shape = ',
...
Both parameters in isolation are **trunk**-like so their interaction determines **scale** and **location**. I think that this makes them **messy**.
...
shinyApp(
  fluidPage(
    sidebarLayout(
      ...
      sidebarPanel(
        width=4,
        sliderInput("gamma_shape", label = NULL, pre='shape = ',
                    min = 0, max = 50, value = 5, step = 0.1),
        sliderInput('gamma_rate', label = NULL, pre='rate = ',
...
shinyApp(
  fluidPage(
    sidebarLayout(
      ...
      sidebarPanel(
        width=4,
        sliderInput("sweibull_shape", label = NULL, pre='shape = ',
                    min = 0, max = 10, value = 2.2, step = 0.1),
        sliderInput('sweibull_scale', label = NULL, pre='scale = ',
