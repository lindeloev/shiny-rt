...
<!--
TO DO NOW:
 * Update everything: cheat sheet, shiny server files, and GitHub.
 * Move to GitHub instead of lindeloev.net
...
All too often, researchers use the normal distribution to model reaction times. I made this document to lower the burden of shifting to better distributions. Because the normal distribution is so poor for RTs, the reward of shifting will be great! 

The cheat sheet below gives an overview (get it in [PNG](http://github.com/lindeloev/shiny-rt/rt_cheat_sheet.png) or [PDF](http://github.com/lindeloev/shiny-rt/rt_cheat_sheet.pdf)). You can also skip straight to the  [interactive part](#data) or to the [applied code example](#code).


<hr />
[![](rt_cheat_sheet.png)](http://github.com/lindeloev/shiny-rt/rt_cheat_sheet.pdf)
<hr />
...
sidebarLayout(
  ...
  # Placeholder for histogram
  mainPanel(
    plotOutput(outputId = "histogram", width='100%', height='100%')
  )
)
```


```{r, context="server", echo=FALSE, eval=FALSE}
pollData = reactivePoll(UPDATE_INTERVAL, session,
  function() last_data_update,
  function() ''
)
